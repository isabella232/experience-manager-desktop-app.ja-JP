...
**一括アップロードを示すアプリケーションアイコン** アプリケーションアイコンは、バルクアップロードが「転送」オーバーレイを表示して進行中であることを示します。

**更新の競合の通知**。 アセットの更新を試みる際に競合が検出されると、通知が表示されるので、ステータスウィンドウを監視する必要がなく、ユーザーが確認できます。 アプリケーションの起動時に競合が漏れなくチェックされるので、競合を解決できます。
...
**再開可能なアップロード**：特に大きいファイルのアップロードにおいて、新しい Asset Status ウィンドウでアップロードを一時停止／再開できます。

**アセットステータスウィンドウの改善**&#x200B;改善されたアセットステータスウィンドウには、アセットに関する次の情報が表示されます。

[!UICONTROL Changes]

* 現在キューに登録されている変更を表示します。

* 進行状況バー、転送率、合計ファイルサイズ、これまでに転送されたサイズなど、進行中のアップロードを表示します。

* 完了したアップロードは、合計転送率と最終速度で表示されます。

* アップロードに失敗すると、エラーメッセージと共に表示され、可能な場合は情報が転送されます。

* 3回失敗したアップロードは、エラーメッセージを表示します。

* 競合するファイルにはユーザーがクリックできるアイコンが表示されます。アイコンをクリックすると、説明と 2 つのオプションが示されたダイアログが表示されます。

   * [!UICONTROL Keep Mine] 即座にファイルをサーバーにアップロードします。

   * [!UICONTROL Overwrite Mine] 即座にローカルファイルを削除し、新しいコピーをサーバーからダウンロードします。

[!UICONTROL Downloads]

* これまでに転送された転送率やサイズなど、進行中のダウンロードを表示します。

* 合計転送数、最終速度、クリック時にファイルを開くアイコンが表示される完了したダウンロード（単一のファイルのみ）。

* 失敗したダウンロードがエラーメッセージと転送情報が表示される（可能な場合）。

* フッターには、ダウンロードされた合計ファイル数と平均転送速度が表示されます.

* Experience ManagerアセットWeb UIから複数のファイルを開くか編集する場合は、それらがグループ化されます。 例えば、myasset.jpegとその他4つのファイル。

* When downloading InDesign Documents including linked assets that are stored in AEM Assets, the desktop app will download all of the linked Assets first, before opening the [!UICONTROL Adobe InDesign] document and indicate the download of linked assets. 例えば、5/24。
...
* 進行状況バーと、現在転送中のファイルの名前を含む進行中のアップロードを表示します。

* 進行中のアップロードではアイコンが表示され、このアイコンをクリックするとアップロードがキャンセルされます。転送は、現在転送中のファイルが完了した後に停止します。。

* 失敗した転送プロセスは、エラーメッセージが表示されます（転送全体が失敗した場合のみ）。
...
AEMに保存されているリンクされたアセットを含むInDesignドキュメントで作業する際のユーザーエクスペリエンスを向上させるため、デスクトップアプリは、これらのリンクされたファイルをローカルキャッシュにプリフェッチしてからダウンロードしてInDesignドキュメントを開きます。 これにより、ユーザーはリンクされているファイルをローカルで利用できるようになり、InDesign の「リンク」パネルでこれらのファイルにアクセスするときに長い時間待つ必要がなくなります。このプリフェッチ機能は、AEM がリンクをサーバー側で認識した場合にのみ動作します。認識されたリンクを含むアセットでは、InDesign アセットのプロパティビューに「参照」のリストが表示されます。
...
* ネットワーク：
   ...
   * (Mac).pacプロキシ設定(CQ-4233430)を使用したAEMへの接続の追加サポート。

   * (Win)アドバンス — ログインURLダイアログの問題を修正しました。(CQ-4236061)

* バグの修正:

   * More Asset Info ダイアログでアクションバーが表示されないことがありました（CQ-4208540）。

   * (Win)AEM AssetsUIから以前のバージョンに戻した後に、ファイルを同期できるようになりました。(CQ-4216411)
...
* 安定性：
   ...
   * 多数のファイルが要求された場合の安定性が向上しました。(CQ-4224212)

   * 追加のチェックによるアセットの更新を改善しました。(CQ-4228291)

* キャッシュ:

   * Photoshop、InDesign、ファインダーのファイルを開く際のディスクエラーと開く際の問題を修正しました。(CQ-4223993、CQ-4217603、CQ-4223717)

   * サイズゼロのバイナリを回避するためのキャッシュを改善しました。(CQ-4216599)

* ログイン： 特別な設定（ローカルで発行される証明書など）では、strictSSLを無効にして接続を許可します(CQ-4223949)。
...
* インストールとアンインストール：
   ...
   * [Windows 32bit] InstallerはMicrosoft .NET Framework v. 4.5 (CQ-4218084)のインストールに失敗しました。

   * (Mac)デスクトップアプリケーションファイルを完全に削除する手動のスクリプト(CQ-4216489)。
...
* バグの修正：
   ...
   * AEMのタイムラインから古いバージョンに戻しても、Finder(CQ-4205194)のアセットは更新されません。

   * 大きなネストされたディレクトリを参照すると、デスクトップアプリがクラッシュする問題を修正しました。(CQ-4208539)

   * デスクトップアプリケーションのマウントポイントが/Volumes/DAMになり、すべてのユーザーに対して一貫性を持つようになりました。(CQ-4208159)

   * 最初にファイルをInDesignに配置すると、更新の警告が表示されます(CQ-4207454)。
...
* ネットワークの問題に対する安定性と回復力の向上。
   ...
   * キャッシュされたファイルの処理の向上(CQ-4204336、CQ-4206263)

   * サイズが2 GBを超える大きなファイルのダウンロード/アップロードの処理が改善されました。(CQ-4206438)

   * Finderで多数のファイルを移動または名前変更する場合に発生する「エラー36」を修正しました。(CQ-4204640)

* AEM Serverとのネットワーク通信の最適化(CQ-4204974、CQ-100903)。

* Creative CloudアプリでAEMアセットを開き、配置、保存する際の信頼性を向上しました。(CQ-4203968、CQ-4205511、CQ-103543、CQ-4207141、CQ-90)980)。

* サポート性の向上： 」オプションを使用してキャッシュをクリアし、ログに簡単にアクセスできるようにします(CQ-4202541、CQ-4204673)。

* その他の修正：
   * 名前に日本語の文字を含むアセットやフォルダー／英語以外の言語設定のサポートの改善（CQ-4195433、CQ-4205793、CQ-4199446）。

   * SSLを使用したログインの処理の改善(CQ-4200217)

   * より信頼性の高い共有マウント(CQ-4200793)

   * 安定性に関する様々な改善点(CQ-4207539、CQ-4200378)。

   * 環境設定でのAEM AssetsURLの処理の改善(CQ-97388)

### AEM デスクトップアプリケーション 1.4 以降で使用可能な機能強化 {#Enhancements-Available-Since-AEM-Desktop-App-14}

* タッチ操作対応UIの新しい作成/フォルダーをアップロードアクションを使用して、階層フォルダーのアップロードを簡略化しました。
...
**フォルダーのアップロード。**&#x200B;より大きな階層フォルダーを AEM にアップロードする場合は、Finder またはエクスプローラーレベルからマウントされた AEM リポジトリにコピー／ドラッグ＆ドロップするのではなく、新しいフォルダーのアップロード機能を使用することをお勧めします。フォルダーのアップロード機能を使用する場合、デスクトップアプリケーションが AEM と直接通信するので、プロセス全体をより細かく制御できます。
