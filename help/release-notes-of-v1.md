...
**接続切断時の対処**. バルクアップロード中に接続が切断されると、アップロードは中止されますが、後で再開できます。[!UICONTROL Retry]オプションを使用して、失敗した個々のファイルのアップロードを再試行できます。
...
* Windows：
   ...
   * 切断時にバッチアップロードを再開した後、特定の状況下で、失敗したアップロードに対して[!UICONTROL Retry]オプションを選択すると、アプリがクラッシュする場合があります(CQ-4251884)(Win)
