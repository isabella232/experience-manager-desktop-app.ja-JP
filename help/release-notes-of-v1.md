...
* 一般：
   * パスのないサーバーを指すには、サーバーURLが必要です（例：`http://server`、`https://server`、`http://server:port`、`https://server:port`）。 /content/dam 以外のコンテキストパスやサブフォルダーはサポートされていません（CQ-89343、CQ-87272）。
* ファイル名／ローカリゼーション：
