...
/の設定で、アップロードの同時実行（加速）を制 **[!UICONTROL Preferences]** 御で **[!UICONTROL Upload acceleration]** きます。 通常、同時実行性が高いほど、アップロードが高速になりますが、リソースを大量に消費し、ローカルマシンの処理能力を大幅に消費する可能性があります。 システムの速度が遅い場合は、concurrencyの値を低くしてアップロードを再試行します。
