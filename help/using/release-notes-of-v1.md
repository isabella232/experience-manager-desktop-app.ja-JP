...
ローカルマシンにインストールする AEM デスクトップアプリケーションのバージョンによっては、AEM サーバーの特定のバージョンや、サーバー側に追加のコンポーネント（サービスパック、ホットフィックスまたは機能パック）が必要になります。AEMサーバーに初めて接続する前に、サーバーが正しく設定されていることを確認してください。 不明な点がある場合は、AEM管理者にお問い合わせください。
...
**ローカル編集／チェックアウト**：アセットに保存される変更内容の自動アップロードをステータスウィンドウで無効にできます。これにより、ユーザーは引き続きファイルの操作と変更の保存を行い、準備が整ったら、すべての変更をアップロードすることにします。

**簡略化された Asset Status ウィンドウ**. ステータスウィンドウが簡略化されました。現在は、「[!UICONTROL Uploads]」タブに、個々のアセットと、フォルダーまたはバルクアップロードの両方が表示されます。以前の「バルクアップロード」タブは削除されました。
...
* ユーザーがクリックできるアイコン付きの競合するファイル。 アイコンをクリックすると、説明と 2 つのオプションを含むダイアログが表示されます。
...
**バックグラウンド操作のステータスを示すアイコン**

アプリケーションアイコンは、バックグラウンド操作の状態を示し、ユーザーに視覚的なキューを提供します。 例えば、アプリケーションが AEM に接続されていない場合、アイコンはグレーアウトされ、アクティブなアップロードがある場合、「同期」を示すオーバーレイが表示されます。
...
* キャッシング：以下を含む、AEM デスクトップアプリケーションのキャッシュを管理するための新しい UI（CQ-4208690）
   ...
   * 新しい UI でキャッシュのクリアボタンが使用できるようになりました。

* ログイン：(Windows)SAML および SSL を使用するように設定されたAEMインスタンスへのログインが修正されました (CQ-4216353)
...
* バグの修正：
   ...
   * アセット名の変更中に発生することがあったエラー 43 の問題を修正しました (CQ-4207900)。
...
この動作は、ローカルディスク上のアセットとAEM Desktop のマウントされたディレクトリ内のアセットに対して同じですが、次の例外があります。

* 配置されたアセットが別のユーザーによって変更された場合、そのアセットを含むドキュメントを他のユーザーが初めて開いたときに警告アイコンが表示されます。 これは、配置されたアセットが既にローカルにキャッシュされている場合にのみ発生します。
...
* タッチ操作対応 UI の新しい作成／アップロードフォルダー操作による階層フォルダーのアップロードを簡略化しました。
   ...
   * デスクトップアプリケーションは、デスクトップ上の特定のフォルダー階層をバックグラウンドで走査し、ファイルをAEM Assetsにアップロードします
   * ユーザーは、進行中の操作の進行状況バーを備えた新しいアップロードキューステータスウィンドウで進行状況を監視できます
   * アップロードキューステータスで提供されるトラブルシューティング情報も改善されました（サーバーへの接続がない、など）。
* 「チェックアウト」操作と「開く」操作を 1 つに組み合わせた、タッチ UI の新しい編集アクション
* タッチ操作対応 UI でのデスクトップ関連アクションのグループ化を最適化しました (AEM 6.3)
* 最新の OS リリースとの互換性を向上
* お客様から報告された修正

### AEM デスクトップアプリケーション 1.3 以降で使用可能な機能強化 {#Enhancements-Available-Since-AEM-Desktop-App-13}

* 効率を向上。 ネットワーク操作が完了するまでの待ち時間を短縮できます。
* Finder の統合が強化され、サムネールなどの機能への安定性とアクセスが向上しました。
* キャッシュとパフォーマンスの向上。
* デスクトップアプリケーション（PS、ID、AI など）から直接保存する機能が強化されました。
...
* ファイルは、最初にローカルに保存され、事前定義された時間（30 秒）が経過した後で、バックグラウンドでAEMにアップロードされます。 これにより、ファイルの保存にかかる時間が短縮されます。
* 中間ファイル操作を使用してファイル（部分保存と一時ファイル）を保存するデスクトップアプリケーションの処理が改善され、AEM Asset タイムラインに正しいバージョンとアセットアップロード情報が表示されるようになりました。
* バックグラウンドのアップロードタスクのステータスを追跡するために提供されるダイアログ。

## 変更について {#list-of-changes}

### Mac 上のマウントポイント {#mount-point-on-mac}

MacOS 10.12(Sierra) 以降、Appleは、ネットワークドライブおよびデバイスのマウントに使用する/Volumes フォルダーの権限を、より制限的に変更しました。 新しいマウントポイントを作成するには、管理権限が必要でした。 この問題は、MacOS 10.12.5で修正されました。
...
* 両方のプラットフォーム（Windows および Mac）：
   * アセットにアクセス/ダウンロードし、変更をAEMにアップロードするためのプロトコルが、ネイティブAEMプロトコル（HTTP ベースの RESTful プロトコル）に変更されました。 ネットワークの運用をより詳細に制御し、ネットワークインフラストラクチャとの互換性を高めます。
...
* 第 1 段階では、ファイルはローカルに保存されます。 これにより、ファイルを保存するユーザーは、ファイルがAEMに完全に転送されるまで待つ必要がなくなり、ファイルが保存されるとすぐに作業を再開できます。
...
**Finder で「アイコンプレビュー」をオフにする：** 特にネットワーク接続の不十分な場合に、Finder で大きなフォルダを効率的に参照するには、「アイコン」と「アイコンプレビュー」の両方がオフになっていることを確認します。 そうしないと、小さいプレビューを生成するために Finder がフォルダー内の各アセットのダウンロードを開始するので、パフォーマンスが低下し、帯域幅の使用量が大きくなる可能性があります（CQ-4219779）。

* Finder で、AEM Assetsの共有ネットワークフォルダーに移動します。
* DAM マウントポイントを右クリック
* 表示オプションの表示を選択
* 「アイコンプレビューを表示」の選択を解除します
* 「デフォルトとして使用」をクリックします。

**新しい AEM サーバーに接続するときにキャッシュをクリーンアップする：**&#x200B;デスクトップアプリケーションが、同じ URL を持つ別の AEM サーバーに接続するとき、キャッシュは自動的に消去されません。キャッシュを手動でクリアして、適切な操作を実行する。 これは通常、同じ URL で実行中に AEM インストール環境を置き換えることができるテストで発生します（CQ-4216982）。
...
* チェックインおよびチェックアウト：
   ...
   * 一部のアプリケーションで読み取り専用フラグが正しくサポートされず、あるユーザーがチェックアウトしたファイルを別のユーザーが保存できてしまいます。変更されたファイルは、他のユーザーがファイルをチェックインするまで転送されません。 AEM では、両方の変更がアセットの別のバージョンとして有効になります（CQ-89551、CQ-87572、CQ-89615）。
   * チェックアウトステータスと読み取り専用ステータスは、Finder で個別に報告されます。 これにより、ユーザーがアセットをチェックアウトすると 2 つの鍵アイコンが表示されます（CQ-89507）。
* Finder の統合：
   ...
   * 手動でのフォルダの再読み込みが常に機能しない。 回避策：フォルダーが自動的に更新されるまで 30 秒待ちます（CQ-97389）。
